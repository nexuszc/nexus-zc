  ...
  try {
    ...
    const earlyReturn = async (reply: string) => {
      ...
      const tgMessage = reply.length > LIMIT
        ? reply.slice(0, LIMIT) + "... (truncated ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ full version saved to Nexus memory)"
        : reply;
    ...
    if (msgLower.startsWith("new client:") || msgLower.startsWith("add client:")) {
      ...
      try {
        ...
        if (error) {
          ...
          return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Failed to create client: ${error.message}`);
        }
        await logUsage(supabase, "new client", true, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client brain created for ${clientName} (ID: ${newClient.id})\n\nSet up their context:\nÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ "client context: ${clientName} | deal: rev_share | offer: [their offer] | goals: [their goals]"\nÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ "assign va: ${clientName} | va: [VA name]"`);
      } catch (err: any) {
        await logUsage(supabase, "new client", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Failed to create client: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("client context:")) {
      ...
      try {
        ...
        if (!client) {
          ...
          return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client "${clientName}" not found. Create them first with "new client: ${clientName}"`);
        }
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Context updated for ${clientName}.`);
      } catch (err: any) {
        await logUsage(supabase, "client context", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Context update failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("assign va:")) {
      ...
      try {
        ...
        if (!client) {
          ...
          return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client "${clientName}" not found.`);
        }
        if (!vaName) {
          await logUsage(supabase, "assign va", false, Date.now() - start, channel);
          return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ VA name required. Format: "assign va: ${clientName} | va: [name]"`);
        }
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${vaName} assigned to ${clientName}.`);
      } catch (err: any) {
        await logUsage(supabase, "assign va", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Assign VA failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("provision:")) {
      ...
      if (client.provision_status === "live") {
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${clientName} is already provisioned at https://${client.slug}.nexuszc.com`);
      }
      ...
      return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Provisioning ${clientName}...\n\nI'll message you when the site is live. This takes about 60 seconds.`);
    }

    // ================================================================
    // APPROVE COMMAND ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ merge dev ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ main
    // ================================================================
    if (msgLower.startsWith("approve:") || msgLower === "approve") {
      ...
      if (!improvement) return earlyReturn("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No pending dev improvements found to approve.");
    ...
    if (msgLower.startsWith("reject:") || msgLower === "reject") {
      ...
      const improvement = improvements?.[0];
      if (!improvement) return earlyReturn("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No pending dev improvements to reject.");
      ...
      return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Rejected "${improvement.title}". Dev branch reset to main.`);
    }
...
Be direct, specific, and actionable. Reference actual data from above.`;
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ NEXUS AUDIT\n\n${audit}`);
      } catch (err: any) {
        await logUsage(supabase, "nexus audit", false, 0, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Audit failed: ${err.message}`);
      }
    }

    // ================================================================
    // HEAL COMMAND ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ trigger health-monitor immediately
    // ================================================================
    ...
    if (msgLower === "nexus status" || msgLower === "status: nexus") {
      ...
      let statusMsg = "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ§ NEXUS STATUS\n\n";

      if (inDev?.length) {
        statusMsg += `IN DEV (waiting for approval):\n`;
        statusMsg += inDev.map((i: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${i.title} (~${i.estimated_minutes}min fix)\n  ${i.recommended_fix}`).join("\n") + "\n\n";
      ...
      if (recent?.length) {
        ...
        statusMsg += recent.map((h: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${h.function_name}: ${h.status}`).join("\n");
      }
    ...
    if (msgLower === "done all") {
      ...
      return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ All tasks marked done.`);
      ...
      return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Task marked done: "${taskDesc}"`);
    }
    ...
    if (msgLower.startsWith("search:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Search: ${query}\n\n${summary}`);
      } catch (err: any) {
        await logUsage(supabase, "search", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Search failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("summarize:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Summary of ${url}\n\n${summary}`);
      } catch (err: any) {
        await logUsage(supabase, "summarize", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Summarize failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("draft email:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ§ Email draft to ${to}\nSubject: ${subject}\n\n${draft}\n\n---\nTo send: "send email: ${to} | subject: ${subject} | body: [paste or edit above]"`);
      } catch (err: any) {
        await logUsage(supabase, "draft email", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Draft failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("send email:")) {
      ...
      if (!GMAIL_CLIENT_ID) {
        ...
        return earlyReturn("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Gmail not configured yet. Email draft saved to memory. Set up Gmail API keys to enable sending.");
      }
      ...
      try {
        ...
        return earlyReturn(result ? `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Email sent to ${to}` : `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Failed to send email. Check logs.`);
      } catch (err: any) {
        await logUsage(supabase, "send email", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Send failed: ${err.message}`);
      }
    }
    ...
    if (matchedDoc) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ ${docType.charAt(0).toUpperCase() + docType.slice(1)}: ${subject}\n\n${doc}`);
      } catch (err: any) {
        await logUsage(supabase, `generate ${docType}`, false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Document generation failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("remind me:")) {
      ...
      if (!fireAt) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Couldn't parse time. Try: "remind me: [what] | in: 2 hours" or "in: 3 days"`);
      const chatId = external_id || "";
      try {
        await supabase.from("reminders").insert({
          chat_id: chatId,
          message: `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ° Reminder: ${reminderText}`,
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ° Reminder set: "${reminderText}"\nFires: ${fireAt.toLocaleString("en-US", { timeZone: "America/Denver" })} MT`);
      } catch (err: any) {
        await logUsage(supabase, "remind me", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Reminder failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("research:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  Research: ${target}\n\n${research}`);
      } catch (err: any) {
        await logUsage(supabase, "research", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Research failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("competitors:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Competitive Analysis: ${market}\n\n${analysis}`);
      } catch (err: any) {
        await logUsage(supabase, "competitors", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Competitive analysis failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("report:")) {
      ...
      try {
        const { data: client } = await supabase
          ...
          .order("created_at", { ascending: false })
          .limit(1).maybeSingle();
        if (!client) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Client "${clientName}" not found.`);
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Client Report: ${client.name}\n\n${report}`);
      } catch (err: any) {
        await logUsage(supabase, "report", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Report failed: ${err.message}`);
      }
    }
...
Format as:
ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ SNAPSHOT: ${client.name}
...
Be specific. Reference actual numbers.` }],
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Snapshot failed: ${err.message}`);
      }
    }
    ...
    if (msgLower === "prioritize tasks" || msgLower === "prioritize") {
      ...
      try {
        ...
        if (!openTasks?.length) return earlyReturn("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No open tasks ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ you're clear.");
        ...
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: `You are Zach's Chief of Staff. Prioritize these open tasks by urgency and business impact.\n\nTASKS:\n${taskList}\n\nReturn a prioritized list with this format:\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ´ URGENT (do today):\n1. [task] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ [why urgent]\n\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡ IMPORTANT (this week):\n2. [task] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ [why important]\n\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¢ QUEUE (when time allows):\n3. [task] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ [reasoning]\n\nBe direct. Explain your reasoning briefly.` }],
          }),
        });
        const data = await res.json();
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ TASK PRIORITY\n\n${prioritized}`);
      } catch (err: any) {
        await logUsage(supabase, "prioritize tasks", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Prioritization failed: ${err.message}`);
      }
    }
      ...
      try {
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: `Give a realistic time and effort estimate for this task from the perspective of Zach Curtis, a multi-venture entrepreneur with AI tools and VAs available.\n\nTASK: ${task}\n\nFormat:\nÃÂÃÂ¢ÃÂÃÂÃÂÃÂ±ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Time estimate: [X hours/days]\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ§ Effort: [low/medium/high]\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Steps: [3-5 concrete steps]\nÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Risks: [what could slow this down]\nÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡ Shortcut: [how to do this faster with AI or VA delegation]` }],
          }),
        });
        const data = await res.json();
        const estimate = data?.content?.[0]?.text || "Could not estimate.";
        await logUsage(supabase, "task estimate", true, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ±ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ ESTIMATE: ${task}\n\n${estimate}`);
      } catch (err: any) {
        await logUsage(supabase, "task estimate", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Estimate failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("sprint plan:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¯ SPRINT PLAN\n\n${plan}`);
      } catch (err: any) {
        await logUsage(supabase, "sprint plan", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Sprint plan failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate invoice:")) {
      ...
      try {
        ...
        await supabase.from("generated_docs").insert({ doc_type: "invoice", client_id: client?.id || null, title: `Invoice ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${clientName} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${amount}`, content: invoice });
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ¾ INVOICE GENERATED\n\n${invoice}`);
      } catch (err: any) {
        await logUsage(supabase, "generate invoice", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Invoice failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate contract:")) {
      ...
      try {
        ...
        await supabase.from("generated_docs").insert({ doc_type: "contract", client_id: client?.id || null, title: `Contract ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${clientName} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${forServices}`, content: contract });
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ CONTRACT GENERATED\n\n${contract}`);
      } catch (err: any) {
        await logUsage(supabase, "generate contract", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Contract failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("follow up:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¬ FOLLOW UP: ${target}\n\n${followUp}`);
      } catch (err: any) {
        await logUsage(supabase, "follow up", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Follow-up failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("status update:")) {
      ...
      try {
        ...
        await supabase.from("generated_docs").insert({
          ...
          title: `Status Update ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${subject} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${new Date().toLocaleDateString()}`,
          content: update,
        });
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ STATUS UPDATE\n\n${update}`);
      } catch (err: any) {
        await logUsage(supabase, "status update", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Status update failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate sop:")) {
      ...
      try {
        ...
        await supabase.from("generated_docs").insert({ doc_type: "sop", client_id: null, title: `SOP ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${process}`, content: sop });
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ SOP: ${process}\n\n${sop}`);
      } catch (err: any) {
        await logUsage(supabase, "generate sop", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ SOP failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate pitch:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Pitch failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate case study:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Case study failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("generate ad copy:")) {
      ...
      try {
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            model: "claude-sonnet-4-5", max_tokens: 800,
            messages: [{ role: "user", content: `Write high-converting ad copy for: ${service}\nPLATFORM: ${platform}\n\nWrite 3 ad variations:\n\nAD 1 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Pain-focused:\nHEADLINE: [max 40 chars]\nBODY: [max 125 chars]\nCTA: [action button text]\n\nAD 2 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Results-focused:\nHEADLINE: [max 40 chars]\nBODY: [max 125 chars]\nCTA: [action button text]\n\nAD 3 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Curiosity/hook:\nHEADLINE: [max 40 chars]\nBODY: [max 125 chars]\nCTA: [action button text]\n\nKeep it punchy. No fluff. Speak to the target audience's real pain.` }],
          }),
        });
        const data = await res.json();
        const adCopy = data?.content?.[0]?.text || "Could not generate ad copy.";

        await supabase.from("generated_docs").insert({ doc_type: "ad_copy", client_id: null, title: `Ad Copy ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${service} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${platform}`, content: adCopy });
        await logUsage(supabase, "generate ad copy", true, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ£ AD COPY: ${service} (${platform})\n\n${adCopy}`);
      } catch (err: any) {
        await logUsage(supabase, "generate ad copy", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Ad copy failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("calculate roi:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ROI calc failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("pricing calculator:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Pricing calc failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("save knowledge:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  Knowledge saved: "${topic}"`);
      } catch (err: any) {
        await logUsage(supabase, "save knowledge", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Save failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("recall knowledge:")) {
      ...
      try {
        ...
        if (!results?.length) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No knowledge found for "${topic}"`);
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  KNOWLEDGE: ${topic}\n\n${knowledge}`);
      } catch (err: any) {
        await logUsage(supabase, "recall knowledge", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Recall failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("learn from:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  LEARNED: ${topic}\n\n${learned}`);
      } catch (err: any) {
        await logUsage(supabase, "learn from", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Learn failed: ${err.message}`);
      }
    }
    ...
    if (msgLower === "nexus brain dump" || msgLower === "brain dump") {
      ...
      try {
        ...
        const fullDump = `NEXUS BRAIN DUMP ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${new Date().toLocaleString()}\n\n` +
          `KNOWLEDGE BASE (${knowledge?.length || 0} items):\n${(knowledge || []).map((k: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${k.topic}`).join("\n") || "Empty"}\n\n` +
          `ACTIVE CLIENTS (${clients?.length || 0}):\n${(clients || []).map((c: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${c.name} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${c.deal_type || "no deal"} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${c.status} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ site: ${c.provision_status}`).join("\n") || "None"}\n\n` +
          `OPEN TASKS (${openTasks?.length || 0}):\n${(openTasks || []).map((t: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${t.content.slice(0, 100)}`).join("\n") || "None"}\n\n` +
          `GENERATED DOCS (${docs?.length || 0} recent):\n${(docs || []).map((d: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ [${d.doc_type}] ${d.title}`).join("\n") || "None"}\n\n` +
          `NEXUS IMPROVEMENT QUEUE:\n${(improvements || []).map((i: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${i.title}`).join("\n") || "None"}`;
        ...
        const summary = `ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  NEXUS BRAIN DUMP\n\n` +
          `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Knowledge: ${knowledge?.length || 0} topics\n` +
          `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¥ Clients: ${clients?.length || 0} active\n` +
          `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Open tasks: ${openTasks?.length || 0}\n` +
          `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Recent docs: ${docs?.length || 0}\n` +
          `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ§ Improvements queued: ${improvements?.length || 0}\n\n` +
          `TOP KNOWLEDGE:\n${(knowledge || []).slice(0, 5).map((k: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${k.topic}`).join("\n") || "None"}\n\n` +
          `OPEN TASKS:\n${(openTasks || []).slice(0, 5).map((t: any) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${t.content.slice(0, 80)}`).join("\n") || "None"}\n\n` +
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Brain dump failed: ${err.message}`);
      }
    }
    ...
    if (msgLower === "roofing jobs" || msgLower === "roofing summary") {
      ...
      try {
        ...
        if (msgLower === "roofing summary") {
          ...
          const statusLines = Object.entries(byStatus).map(([s, c]) => `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${s.replace(/_/g, " ")}: ${c}`).join("\n");
          await logUsage(supabase, "roofing_summary", true, Date.now() - start, channel);
          return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ  ROOFING OS SUMMARY\n\nTotal jobs: ${total}\nActive: ${active}\nContract value: $${value.toLocaleString()}\n\nPIPELINE:\n${statusLines || "No jobs yet"}`);
        }

        const active = (jobs || []).filter((j: any) => !["paid", "cancelled"].includes(j.status));
        const lines = active.map((j: any) =>
          `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ ${j.homeowner_name} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${j.property_address}\n  ${j.status.replace(/_/g, " ")}${j.contract_amount ? ` ÃÂÃÂÃÂÃÂ· $${j.contract_amount.toLocaleString()}` : ""}\n  ID: ${j.id.slice(0, 8)}`
        ).join("\n\n");
        await logUsage(supabase, "roofing_jobs", true, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ  ACTIVE ROOFING JOBS (${active.length})\n\n${lines || "No active jobs"}`);
      } catch (err: any) {
        await logUsage(supabase, "roofing_jobs", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Roofing jobs failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("roofing job:")) {
      ...
      try {
        const { data: job } = await supabase
          ...
          .single();
        if (!job) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Job not found: ${jobId}`);
        const msg =
          `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ  JOB: ${job.homeowner_name}\n` +
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Job lookup failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("roofing new:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Roofing job created\n\nHomeowner: ${homeownerName}\nAddress: ${address || "TBD"}\nContractor: ${client?.name || "unassigned"}\nID: ${job.id}\nPortal: app.nexuszc.com/roofing/portal/${job.portal_token}`);
      } catch (err: any) {
        await logUsage(supabase, "roofing_new", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Job creation failed: ${err.message}`);
      }
    }
    ...
    if (msgLower.startsWith("roofing status:")) {
      ...
      try {
        ...
        if (!job) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Job not found: ${jobIdPart}`);
        if (!newStatus) return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Format: roofing status: [job_id] | status: [new_status]`);
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${job.homeowner_name} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${newStatus.replace(/_/g, " ")}`);
      } catch (err: any) {
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ FOCUS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower === "focus" || msgLower === "what should i focus on" || msgLower === "focus now") {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Focus failed: ${err.message}`);
      }
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ STALE CHECK ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    ...
    if (msgLower === "momentum" || msgLower === "project momentum") {
      ...
      try {
        ...
        return earlyReturn(data.stale_projects === 0 ? "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ All projects have recent activity." : `ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ ${data.stale_projects} project(s) going stale. Alert sent.`);
      } catch (err: any) {
        await logUsage(supabase, "momentum_check", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Momentum check failed: ${err.message}`);
      }
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ HEALTH SCORES ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower === "health scores" || msgLower === "client health") {
      ...
      try {
        ...
        const scoreEmoji = (score: number) => score >= 70 ? "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¢" : score >= 40 ? "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡" : "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ´";
        const reply =
          `*Client Health Scores*\n\n` +
          clients.map((c: any) => `${scoreEmoji(c.health_score || 50)} *${c.name}* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${c.health_score || 50}/100`).join("\n") +
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Health scores failed: ${err.message}`);
      }
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ PROJECT UPDATE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower.startsWith("project update:") || msgLower.startsWith("update project:")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ *${project.name}* momentum updated.${milestone ? `\nNext milestone: ${milestone}` : ""}`);
      } catch (err: any) {
        await logUsage(supabase, "project_update", false, Date.now() - start, channel);
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Project update failed: ${err.message}`);
      }
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ CONTRADICTIONS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower === "contradictions" || msgLower === "show contradictions") {
      ...
      try {
        ...
        if (!contradictions || contradictions.length === 0) {
          ...
          return earlyReturn("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No unresolved contradictions in your memory.");
        }
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Contradictions failed: ${err.message}`);
      }
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ APPROVE QUEUED ACTION ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    // Ã¢ÂÂÃ¢ÂÂ APPROVE ALL Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
    if (msgLower === "approve all") {
      ...
      try {
        ...
        const lines = [];
        if (actionIds.length > 0) lines.push(`Ã¢ÂÂ Approved ${actionIds.length} action${actionIds.length > 1 ? "s" : ""}`);
        if (abilityRows.length > 0) lines.push(`Ã°ÂÂÂ¨ Building ${abilityRows.length} abilit${abilityRows.length > 1 ? "ies" : "y"}: ${abilityRows.map((a: { ability_name: string }) => a.ability_name).join(", ")}`);
    ...
    // Ã¢ÂÂÃ¢ÂÂ APPROVE ALL ABILITIES Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
    if (msgLower === "approve abilities") {
      ...
      try {
        ...
        await logUsage(supabase, "approve_abilities", true, Date.now() - start, channel);
        return earlyReturn(`Ã°ÂÂÂ¨ Building ${abilityRows.length} abilit${abilityRows.length > 1 ? "ies" : "y"}:\n${abilityRows.map((a: { ability_name: string }) => `Ã¢ÂÂ¢ ${a.ability_name}`).join("\n")}\n\nYou'll get a Telegram notification for each one when it's ready to test.`);
      } catch (err: any) {
    ...
    if (msgLower.startsWith("approve action ") || msgLower.startsWith("exec ")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Approved: *${queuedAction.action_summary}*\nExecuting...`);
      } catch (err: any) {
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ REJECT QUEUED ACTION ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower.startsWith("reject action ")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Rejected: *${queuedAction.action_summary}*`);
      } catch (err: any) {
    ...
    if (msgLower.startsWith("approve ability")) {
      ...
      try {
        ...
        if (proposal.status === "testing") {
          ...
          return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Deploying *${proposal.ability_name}* to production...`);
        } else {
          await supabase.from("nexus_ability_proposals")
            .update({ status: "approved", approved_at: new Date().toISOString() })
            .eq("id", proposal.id);
          ...
          return earlyReturn(`ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¨ Building *${proposal.ability_name}*...\nI'll notify you when it's ready to test.`);
        }
      } catch (err: any) {
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ REJECT ABILITY ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower.startsWith("reject ability")) {
      ...
      try {
        ...
        return earlyReturn(`ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Ability rejected and removed from queue.`);
      } catch (err: any) {
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ PENDING ACTIONS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
      ...
      try {
        ...
        if (actions && actions.length > 0) {
          ...
          reply += actions.map((a: any) =>
            `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ [${a.id.slice(0, 8)}] P${a.priority} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${a.action_summary}\n  Approve: \`approve action ${a.id.slice(0, 8)}\``
          ).join("\n") + "\n\n";
        }
        ...
        if (abilities && abilities.length > 0) {
          ...
          reply += abilities.map((a: any) =>
            `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ [${a.id.slice(0, 8)}] *${a.ability_name}* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${a.description?.slice(0, 80)}\n  Build: \`approve ability ${a.id.slice(0, 8)}\``
          ).join("\n");
        }

        if ((!actions || actions.length === 0) && (!abilities || abilities.length === 0)) {
          reply = "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ No pending approvals. Nexus is fully caught up.";
        }
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ AUDIT LOG ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
      ...
      try {
        ...
        const reply = `*Nexus Audit Log (last ${logs.length}):*\n\n` +
          logs.map((l: any) => {
            ...
            const emoji = l.outcome === "success" ? "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ" : l.outcome === "failure" ? "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ" : "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ³";
            return `${emoji} [${time}] *${l.engine}* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${l.action_detail.slice(0, 80)}`;
          }).join("\n");
    ...
    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ RESEARCH NOW ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower === "research now" || msgLower === "nexus research") {
      ...
      return earlyReturn("ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¬ Research cycle started. I'll report back with findings in a few minutes.");
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ AGENT NOW ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    if (msgLower === "agent now" || msgLower === "nexus agent" || msgLower === "run agent") {
      ...
      return earlyReturn("ÃÂÃÂ°ÃÂÃÂÃÂÃÂ¤ÃÂÃÂ Agent cycle triggered. Observing, thinking, acting...");
    }

    // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ABILITIES ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
      ...
      try {
        ...
        if (liveAbilities && liveAbilities.length > 0) {
          ...
          reply += liveAbilities.map((a: any) =>
            `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ *${a.ability_name}* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ \`${a.trigger_command}\` (used ${a.usage_count}x)`
          ).join("\n") + "\n\n";
        }
        ...
        if (proposedAbilities && proposedAbilities.length > 0) {
          ...
          reply += proposedAbilities.map((a: any) =>
            `${a.status === "proposed" ? "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡" : a.status === "building" ? "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¨" : "ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂª"} *${a.ability_name}* (${a.status})`
          ).join("\n");
        }

        if ((!liveAbilities || liveAbilities.length === 0) && (!proposedAbilities || proposedAbilities.length === 0)) {
          reply = "ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¡ No self-built abilities yet. Nexus is still identifying gaps. Check back after the first research cycle.";
        }
...
```javascript
// ââ PRIORITIZE_IMPROVEMENTS ââ
if (msgLower.startsWith('/prioritize-improvements')) {
  ...
  try {
    ...
    if (!improvements || improvements.length === 0) {
      ...
      return earlyReturn("â No pending improvements to prioritize.");
    }
...
Be concise and actionable.`;
    ...
    return earlyReturn("â ï¸ Error analyzing improvements backlog.");
  }
}
```
...
async function summarizeSearchResults(query: string, results: any[]): Promise<string> {
  if (!results.length) return "No results found. (SERPER_API_KEY not configured ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ add it to Supabase secrets to enable web search)";
...
2. **Catch naming events.** "let's call this X", "new idea X", "create a project called X" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ extract as NEW project name.
...
6. **Task prefix detection.** If the message starts with "task:" or "TODO:" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ always classify type as "task".
...
- search: [query] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ search the web
- summarize: [url] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ summarize any webpage
- research: [name] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ deep research on a person or company
- competitors: [market] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ competitive analysis
- draft email: [to] | subject: [x] | about: [x] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ draft an email
- send email: [to] | subject: [x] | body: [x] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ send an email
...
- task: [what] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ track a task
- report: [client] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ full client status report
- client snapshot: [name] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ instant status: pipeline, calls, tasks, next move
- prioritize tasks ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ AI-sorted task list by urgency/impact
- task estimate: [task] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ time/effort estimate with shortcuts
- sprint plan: [timeframe] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ achievable sprint plan
- generate invoice: [client] | for: [work] | amount: [x]
- generate contract: [client] | for: [services] | amount: [x]
- follow up: [name] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ smart follow-up based on Nexus memory
- weekly digest: [client] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ weekly update to send to client
- status update: [project] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ project status report
- generate sop: [process] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ standard operating procedure
- generate pitch: [client] | for: [service] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ custom sales pitch
- generate case study: [client] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ results-focused case study
- generate ad copy: [service] | platform: [x] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ad copy in 3 variants
- calculate roi: [project] | revenue: [x] | cost: [x]
- pricing calculator: [service] | market: [x]
- save knowledge: [topic] | [details] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ build knowledge library
- recall knowledge: [topic] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ pull from knowledge base
- learn from: [url] ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ingest and remember webpage
- nexus brain dump ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ export full knowledge snapshot
- nexus status ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ see system health and improvement queue
- approve / reject ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ approve or reject pending dev improvements
...
Task rules:
- task: or TODO: prefix ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ respond ONLY with: "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Task logged: [task]. I'll track this until you mark it done."
- done: prefix ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ respond ONLY with: "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Done: [what was marked complete]."
- done all ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ respond ONLY with: "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ All tasks cleared."`;
