...
### Social Monitoring (added May 30, 2026):
- `social_posts` -- published social posts (platform, content, post_url, thread_url, subreddit, status, engagement jsonb)
- `social_opportunities` -- Reddit threads worth engaging (platform, thread_url, thread_title, thread_body, subreddit, score, draft_reply, status: found/draft_ready/approved/rejected/posted)
- `social_queue` -- outbound LinkedIn + X posts (platform, content, slug, post_title, image_url, scheduled_for, status: pending/approved/posted/rejected, posted_at, post_url, error)
...
- Edge: social-content-generator — pulls latest 5 published seo_posts (uses meta_description + keyword, NOT excerpt — that column doesn't exist), generates LinkedIn (150-word Haiku) + X (<240 char Haiku) posts per blog; schedules LinkedIn +24h, X +12h; inserts to social_queue with slug + post_title; cron job #85 (30 13 * * *); first run: 5 posts → 10 queue items
- Dashboard: RoofingSocial.jsx at /roofing/social — 3 tabs: Reddit Opportunities (score-ordered, approve/reject, expandable draft reply), Post Queue (pending/approved, approve/reject, expandable content, scheduled time), Posted; 4 stat cards (Reddit Ready / Queue Pending / Queue Approved / Posts Live)
- Nav: "Social" card (orange #ff4500) added to RoofingVertical.jsx between Marketing and Sales
